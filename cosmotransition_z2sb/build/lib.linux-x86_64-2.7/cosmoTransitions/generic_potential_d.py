...
"""
def Jb(x):
    y = np.asanyarray(np.real(Jb_exact(x)))
    return y

def Jf(x):
    y = np.asanyarray(np.real(Jf_exact(x)))
    return y
"""

"""
def Jb(x):
    y = np.real(-math.pi**4./45. + math.pi**2*x/12 - math.pi*(np.sqrt(x+0j))**3/6.)
    return y        
...
class generic_potential():
    ...
    def Vr_from_X(self, X, T):
        ...
        bosons_ring = self.boson_ring(X, T)                
        y = self.Vr(bosons, T, bosons_ring)       
        return y    
    ...
    def Vtot(self, X, T, include_radiation=False):
        ...
        fermions = self.fermion_massSq(X)
        y = self.V0(X)
        #y += self.V1(bosons_re, fermions_re)
