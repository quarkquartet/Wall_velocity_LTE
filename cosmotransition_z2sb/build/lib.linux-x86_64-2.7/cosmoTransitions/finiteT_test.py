...
def _Jf_exact(x):
    ...
    if x.imag == 0:
        ...
        return (
            integrate.quad(f1, 0, abs(x))[0] + integrate.quad(f, abs(x), numpy.inf)[0]
        )
...
def _Jf_exact2(theta):
    ...
    if theta >= 0:
        ...
        return (
            integrate.quad(f, abs(theta) ** 0.5, numpy.inf)[0]
            + integrate.quad(f1, 0, abs(theta) ** 0.5)[0]
        )
...
if os.path.exists(_Jf_dat_path):
    ...
    _xf = numpy.linspace(
        numpy.arcsinh(-1.3 * 20), numpy.arcsinh(-20 * _xfmax / _xfmin), 1000
    )
...
if os.path.exists(_Jb_dat_path):
    ...
    # (so that we're not overpopulating the uniteresting region)
    _xb = numpy.linspace(
        numpy.arcsinh(-1.3 * 20), numpy.arcsinh(-20 * _xbmax / _xbmin), 1000
    )
...
a, b, c, d = -(pi**4) / 45, pi * pi / 12, -pi / 6, -1 / 32.0
...
a, b, d = -7 * pi**4 / 360, pi * pi / 24, 1 / 32.0
...
def d3x2K2(k, x):
    ...
    return numpy.nan_to_num(
        x * (y * k * special.kn(1, k * y) - 3 * special.kn(0, k * y))
    )
