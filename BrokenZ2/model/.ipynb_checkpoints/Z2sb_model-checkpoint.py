...
V(h,s) = -muh^2 h^2/2 + lh h^4/4 + mus^2 s^2/2 + ls s^4/4 + lm h^2 s^2/4 ("-" sign for muh2 is convention from 1911.10206)
...
Z2 spontaneously broken at T=0
...
class model(gp.generic_potential):
    ...
    def init(self, m12, m22, l1, l2, lm, v2re):
        ...
        self.Y1 = 0.651016 #2*80.4/246
        self.Y2 = 0.357394 #2*(91.2**2 - 80.4**2)**0.5/246
        self.Yt = 0.97773 #2**0.5*172.4/246 #just curious about 1/6
        self.nwt = 4
        self.nzt = 2
        self.nwl = 2
        self.nzl = 1
        ...
        # Physical parameters
        self.vew = 246.
        self.wew = (2*(-2*l1*m22-lm*m12)/(4*l1*l2-lm**2))**.5
    ...
    def approxZeroTMin(self):
        ...
        """
        return [np.array([self.vew, self.wew])]
    ...
    def counterterm(self, X):
        ...
        if self.delta_lh is None:
            ...
            self.delta_lhs = -self.d2VCW(vphy, T=0.0)[0][1] / (vphy[0]*vphy[1])
            self.delta_muh = 3*self.delta_lh*vphy[0]**2 + self.d2VCW(vphy, T=0.0)[0][0] + 0.5*self.delta_lhs*vphy[1]**2
            self.delta_ls = (self.gradVCW(vphy, T=0.0)[1] - vphy[1]*self.d2VCW(vphy, T=0.0)[1][1]) / (2*vphy[1]**3)
            self.delta_mus = -(3*self.delta_ls*vphy[1]**2 + self.d2VCW(vphy, T=0.0)[1][1] + 0.5*self.delta_lhs*vphy[0]**2)
